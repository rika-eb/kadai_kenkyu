...
def integrate_light():
    ...
    # print(f"[DEBUG integrate_light] dt={dt:.3f}s, ppfd={ppfd:.2f}, increment={increment:.2f}, total={accumulated_umol:.2f}")


# ===== 30秒ごとの表示 =====
def print_status_if_needed():
    """30秒ごとに現在の照度と積算値を表示"""
    global last_print_time, accumulated_umol  # ★ここが重要
    
    now = time.time()
    if now - last_print_time >= PRINT_INTERVAL:
        current_lux = get_lux()
        current_ppfd = get_ppfd()
        # accumulated_umolはグローバル変数から直接読み取る
...
try:
    ...
    while True:
        ...
        print(f"Accumulated: {accumulated_umol / 1_000_000:.3f} mol/m²")

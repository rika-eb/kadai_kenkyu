...
# ===== 探索角度設定 =====
# ★ 360度（8方向）を45度刻みでスキャン
# angle_step: 0=正面, 1=右45°, 2=右90°, 3=右135°, 4=後180°, -3=左135°, -2=左90°, -1=左45°
ANGLE_STEPS = [-3, -2, -1, 0, 1, 2, 3, 4]   # -135°〜+180° (合計8方向)
# スキャン開始位置：正面から左に3ステップ（-135°）
SCAN_START_STEPS = 3
# スキャン終了後（+180°）から正面（0°）へ戻すための左回転数
SCAN_END_STEPS = 3

# ===== 影エリア判定パラメータ =====
STARTUP_DARK_THRESHOLD = 8      # global_max_luxが不十分と判断する閾値
RELATIVE_SHADOW_RATIO = 0.4     # スキャン内最大値の40%以下なら影と判断
MAX_UNIFORM_DARK = 3            # 一様暗スキャン連続N回で強制脱出
ESCAPE_FORWARD_ATTEMPTS = 2     # ★ 強制脱出時の前進試行回数
...
def escape_shadow_area():
    ...
    手順：
      1. 180°回転
      2. 最大 ESCAPE_FORWARD_ATTEMPTS(=2) 回、障害物確認をしてから前進
      3. 各前進後に明るさが改善したか確認
    """
...
def rescan_for_brightest_movable():
    ...
    print("\n=== Re-scanning (360°) for brightest movable direction ===")
    measurements = scan_360()
...
try:
    ...
    print(f"Scan directions:        360° / 8 directions (45° steps)")

...
# ===== 影エリア判定パラメータ =====
STARTUP_DARK_THRESHOLD = 8      # global_max_luxが不十分と判断する閾値（lux）
RELATIVE_SHADOW_RATIO = 0.4     # スキャン内最大値の40%以下なら影と判断
MAX_UNIFORM_DARK = 3            # 一様暗スキャン連続N回で強制脱出
ESCAPE_FORWARD_ATTEMPTS = 2     # 影脱出時の前進試行回数
SHADOW_HISTORY_WINDOW = 10      # 中央値計算に使う直近の履歴件数
SHADOW_MEDIAN_MIN_SAMPLES = 5   # 中央値計算に必要な最低サンプル数

# ===== 探索角度設定 =====
# angle_step: -3=-135°, -2=-90°, -1=-45°, 0=正面,
#              1=+45°,   2=+90°,  3=+135°, 4=+180°(後方)
ANGLE_STEPS = [-3, -2, -1, 0, 1, 2]  # 360度・8方向
SCAN_START_STEPS = 2    # スキャン開始のため左に回転する回数
SCAN_END_STEPS = 2     # スキャン終了後に正面へ戻すため左に回転する回数
...
global_max_lux = 0
global_max_history = []         # 測定した全luxの履歴（最新100件）
...
def update_global_brightness(current_lux):
    """
    測定値を履歴に追加し、global_max_luxを更新する。
    global_max_luxはモードA/B切り替え判定にのみ使用。
    影判定の閾値計算にはget_reference_lux()の中央値を使う。
    """
...
def get_reference_lux():
    ...
    直近SHADOW_HISTORY_WINDOW件の中央値を使うことで、
    一瞬だけ明るい場所を通過した場合の外れ値の影響を排除する。

    履歴がSHADOW_MEDIAN_MIN_SAMPLES未満の場合はglobal_max_luxで代替。
    """
...
def is_in_shadow_area(measurements):
    ...
    if global_max_lux >= STARTUP_DARK_THRESHOLD:
        # ★ global_max_luxではなく中央値ベースの基準を使う
    ...
    if relative_ratio < RELATIVE_SHADOW_RATIO:
        ...
        print(f"  Global max so far: {global_max_lux:.2f} "
              f"(< {STARTUP_DARK_THRESHOLD})")
        return True
...
def escape_shadow_area():
    ...
    手順：
      1. 180°回転
      2. 最大ESCAPE_FORWARD_ATTEMPTS回、障害物確認してから前進
      3. 各前進後に明るさが改善したか確認
    """
    ...
    for attempt in range(ESCAPE_FORWARD_ATTEMPTS):
        print(f"  Step {attempt + 2}: Forward attempt "
              f"{attempt + 1}/{ESCAPE_FORWARD_ATTEMPTS}")
        ...
        if move_forward():
            ...
            print(f"    ✗ Cannot move forward despite distance check passing")
...
def scan_direction():
    """
    45度刻みで8方向（-135°〜+180°）をスキャンして measurements を返す。
    スキャン前後で正面向きを維持する。

    スキャン順序：
      左3回転で-135°へ → 右に45°ずつ測定×8 → 左3回転で正面へ戻す
    """
    measurements = []
    ...
    for i, angle_step in enumerate(ANGLE_STEPS):
        ...
        print(f"  Scan Angle {angle_step * 45:+4d}°: Lux={lux:.2f}, "
...
def estimate_angle_from_lux(current_lux, measurements):
    ...
    if not candidates:
        ...
        print(f"  No interpolation match, using closest: "
              f"angle={closest[0]}, lux={closest[1]:.2f}, "
              f"dist={closest[2]:.1f}cm")
...
def correct_angle_if_needed(expected_lux, expected_angle_step, measurements):
    ...
    if deviation < ANGLE_CORRECTION_THRESHOLD:
        print(f"  → No correction needed "
              f"(deviation < {ANGLE_CORRECTION_THRESHOLD * 100:.0f}%)")
        return expected_angle_step, False
    ...
    print(f"  Raw correction: {raw_correction:.3f} steps "
          f"({raw_correction * 45:.1f}°)")
    ...
    if int_correction > 0:
        for i in range(int_correction):
            ...
            if not temp_safe:
                print(f"  ⚠ Obstacle at step {i+1}, reverting")
                for _ in range(i + 1):
                    rotate_left()
                return expected_angle_step, False
    ...
    if abs(frac_correction) >= 0.1:
        ...
        if temp_safe:
            ...
            print(f"  ⚠ Obstacle before partial rotation, skipping")
...
def rescan_for_brightest_movable():
    """360度再スキャンして移動可能な最も明るい方向を返す。"""
    print("\n=== Re-scanning (-90~90°) for brightest movable direction ===")
    ...
    while True:
        ...
        if avg_lux < _best_global_avg[0] * DARK_RATIO:
            print(f"Darker detected "
                  f"(current:{avg_lux:.2f} < "
                  f"best:{_best_global_avg[0]:.2f}*{DARK_RATIO}) → U-turn")
            rotate_180()
            if move_forward():
                reset_exploration_state()
            continue
        ...
        if not movable:
            print("No movable direction found in 360° scan")
        ...
        # scan_360()後は正面（0°）向きに戻っているのでそのまま回転
        if rotation_angle > 0:
    ...
    while True:
        ...
        if current_ppfd < stay_start_ppfd * PPFD_DROP_RATIO:
            print(f"Light decreased "
                  f"({current_ppfd:.2f} < "
                  f"{stay_start_ppfd * PPFD_DROP_RATIO:.2f}) → re-explore")
            return "reexplore"

        if current_ppfd > target_ppfd * (1 + BRIGHTNESS_TOLERANCE):
            print(f"Too bright "
                  f"({current_ppfd:.2f} > "
                  f"{target_ppfd * (1 + BRIGHTNESS_TOLERANCE):.2f}) → adjust")
            rotate_left()
            if move_forward():
                return "adjust"
...
try:
    ...
    print(f"Scan directions:         -90° ~ 90 5 directions (45° steps)")
    ...
    print(f"Shadow escape threshold (ModeA): "
          f"{SHADOW_ESCAPE_RATIO * 100:.0f}% of median reference "
          f"(window={SHADOW_HISTORY_WINDOW})")
    print(f"Shadow escape threshold (ModeB): "
          f"relative ratio < {RELATIVE_SHADOW_RATIO} or "
          f"uniform dark for {MAX_UNIFORM_DARK} scans")
    print(f"Escape forward attempts: {ESCAPE_FORWARD_ATTEMPTS}")
    print()

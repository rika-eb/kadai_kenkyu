...
def explore(target_ppfd):
    ...
    # 最も明るかった地点を記録
    brightest_ppfd = 0
    steps_from_brightest = 0  # 最も明るい地点から何ステップ移動したか
    ...
    while True:
        ...
        if current_max_ppfd <= previous_max_ppfd * 1.05:  # 5%以上の改善がない
            ...
            if no_improvement_count >= MAX_NO_IMPROVEMENT:
                ...
                # 最も明るかった地点に戻る
                if steps_from_brightest > 0:
                    print(f"Returning to brightest point ({steps_from_brightest} steps back)")
                    rotate_180()
                    for _ in range(steps_from_brightest):
                        move_backward()
                        integrate_light()
                    rotate_180()  # 元の向きに戻す
